...
const insertMovies = (movies: Movie[]) => {
  return (
    ...
    movies
      .map(
        (movie) =>
          `('${escape(movie.imdbId)}','${escape(
            movie.popularity.toString()
          )}','${escape(movie.budget.toString())}','${escape(
            movie.budgetAdjusted.toString()
          )}','${escape(movie.revenue.toString())}','${escape(
            movie.revenueAdjusted.toString()
          )}','${escape(movie.originalTitle)}','${escape(
            movie.homepage
          )}','${escape(movie.tagline ? movie.tagline : "null")}','${escape(
            movie.overview
          )}','${escape(movie.runtime.toString())}','${escape(
